...
void UsdExportImport::AddMaterialWithTexturesIfNotAlreadyAdded(const ON_UUID& matId, const ON_wString& matName, const ON_PhysicallyBasedMaterial* pbrMaterial, const ON_ObjectArray<ON_Texture>& textures)
{
  ...
  for (int i = 0; i < texture_count; i++)
  {
    ...
    //pxr::GfVec4f scaleVec(float(scalex), float(scaley), float(scalez), float(1.0));
    ...
    //usdUVTextureSampler.CreateInput(TfToken("scale"), pxr::SdfValueTypeNames->Float4).Set(scaleVec);
    //pxr::GfVec4f biasVec(float(transx), float(transy), float(transz), float(1.0));
    //usdUVTextureSampler.CreateInput(TfToken("bias"), pxr::SdfValueTypeNames->Float4).Set(biasVec);
    ...
    //transform2d.CreateInput(TfToken("translation"), SdfValueTypeNames->Float2);
