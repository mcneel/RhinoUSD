...
std::vector<ON_wString> UsdShared::GetLayerNames(const CRhinoObject& rhinoObject, const std::vector<ON_wString>& priorLayers)
{
  std::vector<ON_wString> names;

  CRhinoDoc* doc = rhinoObject.Document();
  if (!doc) return names;

  const CRhinoObjectAttributes& attributes = rhinoObject.Attributes();
  ...
  for(const ON_wString& priorLayer : priorLayers)
  {
    names.insert(names.begin(), priorLayer);
  }
